...
> Design notes for **creator-bounce-suppression-python** — a runnable python example that Python example that suppresses creator subscribers after hard bounces before the next digital delivery.
...
- **Feature code** — the actual task: bounce-suppression.
...
- `email.suppression.add` — mapped to `POST /v1/email/suppression/add`.
